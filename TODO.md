...
from meeting
- [ ] different channels 
- [x] Let D1 plot of the single subjects overlapping under the avg plot (also created a scpecific plot scripts to see every subject by itself)
- [ ] run on different stories separately (with different pace)
- [ ] run on Ali's data and on EEG



- [x] DONE 27 noises for autocorrelation - same number of features as in the MEG
- [ ] Run analysis on wordlist time to compare results 
- [ ] run PCR/Lasso/ridge/elasticNet
    - [ ] run again elastic net recursively on subject 1 to tune the best parameters by inspecting every time the results
...
- [x] qsub -v ANALYSIS_NAME=elasticnet_nolock_nooverlap,DRSA_MODELS="Envelope,Phoneme Voicing,Word Frequency,GloVe,GPT Next-Token,GPT Surprisal"      -t 1      s2_submit_C1_subject.sh      -- --regression-method elasticnet --progress-log-every 1 --progress-neural-step 1


- [x] python D1_group_cluster_analysis.py --analysis-name center_on_onset_overlap --subjects $(seq -w 1 27) --simulation-noise --simulation-neural-label "Random Noise 208"


- [x] python D1_group_cluster_analysis.py --analysis-name center_on_onset_overlap --subjects $(seq -w 1 27) --models "Envelope" "Phoneme Voicing" "Word Frequency" "GloVe" "GloVe Norm" "GPT Next-Token" "GPT Surprisal" --simulation-noise --simulation-neural-label "MEG-Like Subsample Shuffle" --simulation-origin meg_like --skip-matrix-clusters


- [x]    qsub -t 1-27 s2_submit_A1_preproc.sh -- --overwrite
- [x]    qsub -t 1-27 s2_submit_A2_concat.sh -- --overwrite
- [x]    qsub s2_submit_C1_subject.sh -- --analysis-name lock_to_onset_overlap --lock-subsample-to-word-onset --allow-overlap
- [x]    qsub -t 1-27 s2_submit_C2_plot.sh -- --analysis-name lock_to_onset_overlap
- [x]    qsub s2_submit_D1_group.sh -- --analysis-name lock_to_onset_overlap

try new dRSA matrix permutation
- [x] python D1_group_cluster_analysis.py --analysis-name lock_to_onset_overlap --subjects $(seq -w 1 27) --models "Envelope" "Phoneme Voicing" "Word Frequency" "GloVe" "GloVe Norm" --n-permutations 1000 --log-level DEBUG --matrix-downsample-factor 4
