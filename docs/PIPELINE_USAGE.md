...
### C2_plot_dRSA.py
Recreates subject-level dRSA figures from the cached matrices/lag curves emitted by C1. For every neural signal it regenerates a 2-column figure (matrix + lag curve per model) with peak annotations, optional bootstrap confidence bands, and descriptive captions.

- **Input** – The metadata JSON stored alongside each subject run plus `*_dRSA_matrices.npy` and, when available, `*_lag_curves.npy`. The loader resolves the file paths recorded in the metadata, falls back to conventional filenames if needed, and inspects both `results/<analysis>/single_subjects/` and `results/<analysis>/simulations/` so a single command can target standard and simulation outputs.
- **Output** – One PNG per neural label stored with the original run (or using the `outputs.plot_target` override from the metadata). Passing `--custom-ylim` produces an additional copy of every plot with the lag axis constrained to the requested range. When simulation runs are discovered, the script also collates their autocorrelation matrices/curves into `<run_id>_sim_autocorrelations_plot.png`.
- **Key options**
  - `subject` (positional) – `sub-XX` or a bare integer (e.g. `01`, `1`).
  - `--analysis-name NAME` – pick a specific analysis folder under `--results-root`; otherwise the most recent directory is reused automatically.
  - `--analysis-id RUN_ID` – skip the subject/resolution/method inference and plot a fully qualified run id.
  - `--resolution`, `--rsa-method` – only used when constructing the default run id (`sub-XX_res100_correlation` etc.).
  - `--results-root PATH` / `--results-dir PATH` – point the loader to the cached outputs; the legacy `--results-dir` bypasses the usual `<analysis>/single_subjects` layout.
  - `--custom-ylim YMIN YMAX` – save an extra set of plots with the lag axes fixed to the provided range (useful for comparing subjects with the same scale).
- **Usage**
  ```bash
  # Preferred: reuse the named analysis folder created during C1
  python C2_plot_dRSA.py sub-01 --analysis-name lexical_mem_2024

  # Regenerate a legacy run by its explicit ID
  python C2_plot_dRSA.py sub-01 --analysis-id sub-01_res100_correlation

  # Emit both free-scale and fixed-scale lag plots
  python C2_plot_dRSA.py sub-07 --custom-ylim -0.05 0.2 --analysis-name lexical_mem_2024
  ```
- **Notes**
  - Lag bootstrap samples (when saved by C1) are resampled on the fly to display confidence intervals.
  - Simulation jobs inherit the same plotting path as the parent subject and additionally contribute to the combined `*_sim_autocorrelations_plot.png` for quick QA.
...
Creates compact grids of subject-specific lag curves for a selected model using the `group_dRSA_summary*.npz` caches produced by D1. Each panel shows the individual curve, the group mean/SEM envelope, optional model autocorrelation overlays sourced from simulation runs, and vertical markers at significant cluster peaks.

- **Inputs** – Group-level caches stored in `results/<analysis>/group_level/` that include `subject_lag_curves` and `significance_masks`. Enabling `--overlay-autocorrelation` also requires simulation lag curves under `results/<analysis>/simulations/`.
- **Outputs** – PNG panels written to `results/<plot-location>/` (or an absolute folder) named `<cache>_<model>_lag_panels.png`. When `--autocorr-ylim` is set, the filename includes a suffix describing the secondary-axis range.
- **Key options**
  - `--analysis-name NAME`, `--results-root PATH` – locate the cached group summaries.
  - `--plot-location SUBDIR` – pick/override the destination for the new figures.
  - `--model "Label"` – exact model label to visualise (must match the cache metadata).
  - `--n-columns INT`, `--x-tick-interval SEC` – control the subplot layout and tick spacing.
  - `--overlay-autocorrelation` – add the model’s simulated autocorrelation curve to every panel; combine with `--autocorr-ylim YMIN YMAX` to plot it on a twin y-axis with fixed bounds.
  - `--log-level` – adjust verbosity (warnings report missing lag curves/clusters).
- **Usage**
  ```bash
  python check_plots_by_subject.py \
    ...
    --autocorr-ylim -0.1 0.4
  ```
- **Notes** – caches lacking subject lag curves are skipped with a warning; rerun `D1_group_cluster_analysis.py` with the single-subject overlay enabled if no figures are produced.
