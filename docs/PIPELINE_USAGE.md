...
- **Key options**
  ...
  - `--simulation` – reuse the cached subsamples but swap the neural input: (1) run MEG×MEG once (neural autocorrelation only), (2) run two seeded random-noise controls (300-D correlation + 1-D euclidean), and (3) run each model as the “neural” source against the full model set. Outputs land in `results/<analysis_name>/simulations/`. If that folder already contains any `_sim_*` metadata for the requested subject, the script exits immediately and prints where to find the existing plots instead of recomputing. (Skip is all-or-nothing: delete or move the previous simulation outputs if you need to regenerate them.)
- **Usage**
  ```bash
